...
| `copy-feature` | 외부 오픈소스·아이디어를 atom-flow로 이식할 때 개념 추출·이식 가능성 판정 | 수동/자동 모두 |

### shared-skills/ — 보조성·범용 (2개)

| 스킬 | 역할 |
|---|---|
| `writing-clearly-and-concisely` | Strunk 글쓰기 원칙 적용. spec/plan/report 작성 단계에서 임의로 호출 가능 |
| `using-atom-flow` | 이 파일. SessionStart hook이 주입할 가이드 본문 |
